...
full_df$docType <- as.factor(full_df$docType)
...
```{r}
# train/test of dtm
train_index <- sample(1:nrow(full_dtm_df), size = 0.7 * nrow(full_dtm_df))
train <- full_dtm_df[train_index,]
test <- full_dtm_df[-train_index,]

# train and test of original df with type
train_index_type <- sample(1:nrow(full_df), size = 0.7 * nrow(full_df))
train <- full_df[train_index_type,]$docType
test <- full_dtm_df[-train_index_type,]
```


Create train/test datasets of the Fake data. 70% will  be the training set and 30% will be the test set
```{r}
#split data into train and test sets 70/30
train_index_fake <- sample(1:nrow(fake_dtm_df), size = 0.7 * nrow(fake_dtm_df))
fake_train <- fake_dtm_df[train_index_fake,]
fake_test <- fake_dtm_df[-train_index_fake,]
...
fake_model <- randomForest(docType ~ ., data = fake_train)
```
