...
## SVM models & Predictions Fake/True data

### Random Forest 

import data again
```{r}
fake_df_all <- read.csv("https://github.com/ltcancel/DATA698/blob/main/Data/Fake.csv?raw=true")
real_df_all <- read.csv("https://github.com/ltcancel/DATA698/blob/main/Data/True.csv?raw=true")
```


Create df for fake data
```{r}
# temp df of fake DF with a document type label
temp_fake <- fake_df
temp_fake$docType <- "Fake"
...
# test remove stopwords from real df before creating corpus
#temp_fake$text <- removeWords(temp_fake$text, stopwords("english"))
?as.corpus
```

Create df for real data
```{r}
# temp df of the real df with a document type label
temp_real <- real_df
temp_real$docType <- "Real"
...
# test remove stopwords from real df before creating corpus
#temp_real$text <- removeWords(temp_real$text, stopwords("english"))
```


Combined datasets if needed
```{r eval=FALSE, include=FALSE}
# combine both datasets to use for modeling
full_df <- rbind(temp_fake, temp_real)


# rearrange column order
full_df2 <- full_df %>%
  select(doc, text, docType)

full_df2$docType <- as.factor(full_df2$docType)

# df of just the document numbers and the doctype
doc_list <- full_df2 %>%
  select(doc, docType)
...
```{r}
# load data as a corpus
full_corp <- VCorpus(VectorSource(full_df$text))
...
# remove english common stopwords
full_corp <- tm_map(full_corp, removeWords, stopwords("english"))
# remove punctuation
full_corp <- tm_map(full_corp, removePunctuation)
urlRemover <- function(x) gsub("http:[[:alnum:]]*","", x)
full_corp<-tm_map(full_corp,content_transformer(urlRemover))
#email remover
emailRemover<-function(x) str_replace(x,"(?<=\\s)[[:alnum:]._-]+@[[:alnum:],.-]{2,}","")
full_corp<-tm_map(full_corp,content_transformer(emailRemover))

# replace "/", "@", and "|" with space
#toSpace <- content_transformer(function(x, pattern) gsub(pattern, " ", x))
#fake_corp <- tm_map(fake_corp, toSpace, "/")
#fake_corp <- tm_map(fake_corp, toSpace, "@")
#fake_corp <- tm_map(fake_corp, toSpace, "\\|")


# eliminate extra white spaces
#fake_corp <- tm_map(fake_corp, stripWhitespace)
# text stemming - reduce words to their root form
#fake_corp <- tm_map(fake_corp, stemDocument)
# remove custom words after seeing the frequency
#removeSpecialChars <- function(x) gsub("-","",x)
#fake_corp <- tm_map(fake_corp, removeSpecialChars)
# remove custom words 
#fake_corp <- tm_map(fake_corp, removeWords, c("said","news","fake","post","''"))
```


Attempt to make a corpus with classification meta data
```{r}
fake_meta <- fake_df$text %>% 
  VectorSource() %>% Corpus() 
meta(fake_meta, "fake") <- 0

real_meta <- real_df$text  %>% 
  VectorSource() %>% Corpus() 
meta(real_meta, "real") <- 1

# combine corpus
full_corp <- c(fake_meta, real_meta)
...
Create DTM of the full corpus and export to dataframe
```{r}
full_dtm <- DocumentTermMatrix(full_corp)

inspect(full_dtm)

#remove sparse terms
full_dtm <- removeSparseTerms(full_dtm, 0.96)

inspect(full_dtm)

full_dtm_df <- as.data.frame(as.matrix(full_dtm))

full_dtm_df <- full_dtm_df %>%
  mutate(doc = row_number())

# add document type label back to df
final_full_dtm_df <- cbind("doc"=doc_list$doc,full_dtm_df,"docType"=doc_list$docType )

head(final_full_dtm_df)
#summary(fake_dtm_df)
```

more cleaning
```{r}
final_full_dtm_df <- final_full_dtm_df[,-2]
final_full_dtm_df %>%
  rename("dont" = `don’t`)
train <- train %>%
  rename("dont" = `don’t`)
train$`else` = NULL
train <- train %>%
  rename("obamas" = `obama’s`)
train <- train %>%
  rename("presidents" = `president’s`)
train <- train %>%
  rename("trumps" = `trump’s`)
train$`next` = NULL
test <- test %>%
  rename("dont" = `don’t`)
test$`else` = NULL
...
```{r}
#shuffle df
final_full_dtm_df <- final_full_dtm_df[sample(1:nrow(final_full_dtm_df)),]
full_df <- full_df[sample(1:nrow(full_df)),]
```

```{r}
set.seed(123)
# train/test of dtm
train_index <- sample(1:nrow(final_full_dtm_df), size = 0.7 * nrow(final_full_dtm_df))
train <- final_full_dtm_df[train_index,]
test <- final_full_dtm_df[-train_index,]

# train and test of original df with type
train_index_type <- sample(1:nrow(full_df), size = 0.7 * nrow(full_df))
train_type <- full_df[train_index_type,]$docType
test_type <- full_df[-train_index_type,]$docType
```

remove columns that cause issues for the model
```{r}
...
```{r}
nb_model <- naiveBayes(docType ~ ., data = train)
...
```{r}
CrossTable(nb_predict, test_type,
...
Compare accuracy
```{r}
accuracy_nb <- sum(nb_predict == test$docType) / nrow(test)
accuracy_rf <- sum(rf_predict == test$docType) / nrow(test)
accuracy_nb
accuracy_rf
...
```{r}
# calculate accuracy
accuracy <- sum(predictions == train_type) / length(train_type)
accuracy
```

Random Forest model
```{r}
fr_model <- randomForest(docType ~., data=train)
```

predict from random forest model
```{r}
rf_predict <- predict(fr_model, newdata = test)

#convert to dataframe
#rf_predict_df <- as.data.frame(rf_predict)
#test_type <- as.data.frame(test_type)

cm <- table(rf_predict, test_type)
cm
CrossTable(rf_predict,test_type,
           prop.chisq = FALSE, prop.t = FALSE,
           dnn = c("Prediction","Actual"))


length(rf_predict_df)
length(test_type)
tail(test_type)
```
...
```{r}
svm_model <- svm(docType ~., data = train, kernel = "linear")

svm_predict <- predict(svm_model, newdata = test)

CrossTable(svm_predict, test_type,
           prop.chisq = FALSE, prop.t = FALSE,
           dnn = c("Prediction","Actual"))
```
